(() => {
  ...
  function updateWeapon(data){ if(!data)return; const id=Math.max(0,Math.trunc(Number(data.WeaponId!==undefined?data.WeaponId:data.weaponId)||0)); const clip=data.AmmoClip!==undefined?data.AmmoClip:(data.ammoClip??data.ammo??0); const total=data.AmmoTotal??data.ammoTotal??data.maxAmmo??data.MaxAmmo??data.max_ammo??data.totalAmmo??data.TotalAmmo??data.ammo_total??0; $('weaponImage').src='images/weapons/'+(HUD_WEAPON_IMAGES[id]||HUD_WEAPON_IMAGES[0]); const ammo=$('ammo'); if(ammo)ammo.style.display=id===0?'none':''; setAmmo(clip,total); }
