(() => {
  ...
  function updateWeapon(data){
    ...
    if(typeof data==='number')data={WeaponId:data};
    if(typeof data==='string'){
      const raw=data.trim();
      if(!raw)return;
      try{data=JSON.parse(raw)}catch(_){const n=Number(raw);data=Number.isFinite(n)?{WeaponId:n}:null}
      if(typeof data==='number')data={WeaponId:data};
    }
    if(!data||typeof data!=='object')return;
    const id=Math.max(0,Math.trunc(Number(data.WeaponId??data.weaponId??data.weapon??data.Weapon)||0));
    const clip=data.AmmoClip??data.ammoClip??data.ammo??data.Ammo??0;
    const total=data.AmmoTotal??data.ammoTotal??data.maxAmmo??data.MaxAmmo??data.max_ammo??data.totalAmmo??data.TotalAmmo??data.ammo_total??0;
    const image=$('weaponImage');if(image)image.src='./assets/CSS/Images/Hud/Weapons/'+(HUD_WEAPON_IMAGES[id]||HUD_WEAPON_IMAGES[0]);
    const ammo=$('ammo');if(ammo)ammo.style.display=id===0?'none':'';
    setAmmo(clip,total);
  }
