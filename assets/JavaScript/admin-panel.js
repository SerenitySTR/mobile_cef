var AdminPanel = {
    ...
    Chat: function(r){var owned=this.Mine(r),closed=r.status==="closed",self=this;return '<div class="admin-chat-head"><button type="button" class="admin-back" data-admin-back>‹ Репорти</button><h2>#'+this.Escape(r.id)+' · '+this.Escape(r.subject)+'</h2><small>'+this.Escape(r.playerName)+' [ID: '+this.Escape(r.playerId)+'] · '+(closed?"Закрито":r.adminId==null?"Вільний":"В роботі")+'</small></div><div class="admin-messages">'+(r.messages||[]).map(function(m){return '<div class="admin-message '+(m.role==="admin"?"admin-own":"")+'"><small>'+self.Escape(m.name)+' · '+self.Escape(m.time)+'</small><div class="admin-bubble">'+self.Escape(m.text)+'</div></div>';}).join("")+'</div>'+(owned?'<div class="admin-composer"><div class="admin-quick">'+this.quickReplies.map(function(q,i){return '<button type="button" data-admin-quick="'+i+'">'+q.label+'</button>';}).join("")+'</div><div class="admin-send-row"><textarea class="admin-textarea" data-admin-draft placeholder="Напишіть повідомлення…"></textarea><button type="button" class="admin-send" data-admin-action="send">➤</button></div><div class="admin-actions"><select class="admin-select"><option>Передати адміну…</option></select><button type="button" data-admin-action="transfer">Передати</button><button type="button" class="admin-primary" data-admin-action="resolve">✓ Гравцю допомогли</button></div></div>':!closed&&r.adminId==null?'<div class="admin-composer"><button type="button" class="admin-primary" data-admin-action="claim">Взяти репорт</button></div>':'<div class="admin-composer admin-muted">Перегляд історії листування</div>');},
    ...
    Click: function(e){var b=e.target.closest("button");if(!b)return;if(b.dataset.adminTab){this.tab=b.dataset.adminTab;this.query="";this.chatOpen=false;this.Render();}else if(b.dataset.adminFilter){this.filter=b.dataset.adminFilter;this.selectedReport=null;this.chatOpen=false;this.Render();}else if(b.dataset.adminReport){this.selectedReport=b.dataset.adminReport;this.chatOpen=true;this.Render();}else if(b.dataset.adminItems){this.itemCategory=b.dataset.adminItems;this.query="";this.Render();}else if(b.dataset.adminBack!==undefined){this.chatOpen=false;this.Render();}else if(b.dataset.adminQuick!==undefined){var q=this.quickReplies[Number(b.dataset.adminQuick)],d=this.Q("[data-admin-draft]");if(q&&d){d.value=q.text;d.focus();}}else if(b.dataset.adminSpawn!==undefined||b.dataset.adminAction!==undefined){this.NotConnected();}},
