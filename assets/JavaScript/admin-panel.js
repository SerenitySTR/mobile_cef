var AdminPanel = {
    ...
    state: { profile:{id:null,name:"",role:""}, stats:{days:[],period:""}, admins:[], tickets:[], commands:[], punishments:[], items:{weapons:[],vehicles:[],skins:[],organizations:[]}, locations:[] },
    quickReplies: [
    ...
    Ticket: function(){var self=this;return this.state.tickets.find(function(r){return String(r.id)===String(self.selectedTicket);});},
    ...
    Scale: function(){if(!this.root)return;var desktop=window.innerWidth>=768;if(!desktop){this.root.style.removeProperty("--admin-panel-scale");return;}this.root.style.setProperty("--admin-panel-scale",Math.min(window.innerWidth*.92/1280,window.innerHeight*.88/720));},
    ...
    Render: function(){if(!this.root)return;var labels={stats:"Статистика",tickets:"Звернення",commands:"Адмін-команди",punishments:"Гайд покарань",items:"Довідник ID",spawns:"Швидкі спавни"};var nav=[["stats","▥"],["tickets","♧"],["commands","›_"],["punishments","▤"],["items","◇"],["spawns","⌖"]],self=this;this.Q("#admin-nav").innerHTML=nav.map(function(n){return '<button type="button" data-admin-tab="'+n[0]+'" class="'+(self.tab===n[0]?"active":"")+'"><span class="admin-icon">'+n[1]+'</span><span>'+labels[n[0]]+'</span>'+(n[0]==="tickets"?'<span class="admin-badge">'+self.state.tickets.filter(function(r){return r.status!=="closed";}).length+"</span>":"")+"</button>";}).join("");this.Q("#admin-title").textContent=labels[this.tab];this.Q("#admin-content").innerHTML=({stats:this.Stats,tickets:this.Tickets,commands:this.Commands,punishments:this.Punishments,items:this.Items,spawns:this.Spawns}[this.tab]).call(this);var m=this.Q(".admin-messages");if(m)m.scrollTop=m.scrollHeight;},
    ...
    FilteredTickets: function(){var self=this,q=this.query.toLowerCase();return this.state.tickets.filter(function(r){var ok=self.filter==="closed"?r.status==="closed":self.filter==="free"?r.status!=="closed"&&r.adminId==null:self.Mine(r);return ok&&[r.id,r.playerName,r.playerId,r.subject].join(" ").toLowerCase().includes(q);});},
<<<<<<< HEAD
    Tickets: function(){var rows=this.FilteredTickets(),self=this;if(!rows.some(function(x){return String(x.id)===String(self.selectedTicket);})){this.selectedTicket=rows[0]?rows[0].id:null;}var r=this.Ticket(),counts={free:this.state.tickets.filter(function(x){return x.status!=="closed"&&x.adminId==null;}).length,mine:this.state.tickets.filter(function(x){return self.Mine(x);}).length,closed:this.state.tickets.filter(function(x){return x.status==="closed";}).length};return '<div class="admin-tabs">'+[["free","Вільні"],["mine","Мої"],["closed","Закриті"]].map(function(x){return '<button type="button" data-admin-filter="'+x[0]+'" class="'+(self.filter===x[0]?"active":"")+'">'+x[1]+" · "+counts[x[0]]+"</button>";}).join("")+'</div><div class="admin-ticket-layout '+(this.chatOpen?"admin-chat-open ":"")+(this.ticketFocus?"admin-ticket-focus":"")+'"><div class="admin-tickets"><input class="admin-search" data-admin-search value="'+this.Escape(this.query)+'" placeholder="Пошук за назвою, ID або описом">'+(rows.map(function(x){return '<button type="button" class="admin-ticket '+(String(x.id)===String(self.selectedTicket)?"active":"")+'" data-admin-ticket="'+self.Escape(x.id)+'"><span>#'+self.Escape(x.id)+' · '+self.Escape(x.waitLabel||"")+'</span><strong>'+self.Escape(x.playerName)+' ['+self.Escape(x.playerId)+']</strong><small>'+self.Escape(x.subject)+'</small></button>';}).join("")||'<div class="admin-empty">Звернень немає</div>')+'</div><div class="admin-chat">'+(r?this.Chat(r):'<div class="admin-empty">Оберіть звернення</div>')+'</div></div>';},
    Chat: function(r){
        ...
        return '<div class="admin-chat-head"><div class="admin-chat-title-row"><div><h2>#'+this.Escape(r.id)+' · '+this.Escape(r.subject||r.title||"Звернення")+'</h2><small>'+this.Escape(r.playerName)+' [ID: '+this.Escape(r.playerId)+'] · '+(closed?"Закрито":r.adminId==null?"Вільний":"В роботі")+'</small></div><button type="button" class="admin-ticket-focus-toggle" data-admin-ticket-focus>'+(this.ticketFocus?"Показати список":"Сховати список")+'</button></div></div><div class="admin-messages">'+messages+'</div>'+controls;
    },
=======
    Tickets: function(){var rows=this.FilteredTickets(),self=this;if(!rows.some(function(x){return String(x.id)===String(self.selectedTicket);})){this.selectedTicket=rows[0]?rows[0].id:null;}var r=this.Ticket(),counts={free:this.state.tickets.filter(function(x){return x.status!=="closed"&&x.adminId==null;}).length,mine:this.state.tickets.filter(function(x){return self.Mine(x);}).length,closed:this.state.tickets.filter(function(x){return x.status==="closed";}).length};return '<div class="admin-tabs">'+[["free","Вільні"],["mine","Мої"],["closed","Закриті"]].map(function(x){return '<button type="button" data-admin-filter="'+x[0]+'" class="'+(self.filter===x[0]?"active":"")+'">'+x[1]+" · "+counts[x[0]]+"</button>";}).join("")+'</div><div class="admin-ticket-layout '+(this.chatOpen?"admin-chat-open":"")+'"><div class="admin-tickets"><input class="admin-search" data-admin-search value="'+this.Escape(this.query)+'" placeholder="Пошук за назвою, ID або описом">'+(rows.map(function(x){return '<button type="button" class="admin-ticket '+(String(x.id)===String(self.selectedTicket)?"active":"")+'" data-admin-ticket="'+self.Escape(x.id)+'"><span>#'+self.Escape(x.id)+' · '+self.Escape(x.waitLabel||"")+'</span><strong>'+self.Escape(x.playerName)+' ['+self.Escape(x.playerId)+']</strong><small>'+self.Escape(x.subject)+'</small></button>';}).join("")||'<div class="admin-empty">Звернень немає</div>')+'</div><div class="admin-chat">'+(r?this.Chat(r):'<div class="admin-empty">Оберіть звернення</div>')+'</div></div>';},
    Chat: function(r){var owned=this.Mine(r),closed=r.status==="closed",self=this,admins=this.state.admins.filter(function(a){return String(a.id)!==String(self.state.profile.id);});var quickMenu=this.quickOpen?'<div class="admin-quick-menu">'+this.quickReplies.map(function(q,i){return '<button type="button" data-admin-quick="'+i+'">'+self.Escape(q.label)+'</button>';}).join("")+'</div>':'';return '<div class="admin-chat-head"><h2>#'+this.Escape(r.id)+' · '+this.Escape(r.subject||r.title||"Звернення")+'</h2><small>'+this.Escape(r.playerName)+' [ID: '+this.Escape(r.playerId)+'] · '+(closed?"Закрито":r.adminId==null?"Вільний":"В роботі")+'</small></div><div class="admin-messages">'+(r.messages||[]).map(function(m){var role=m.role||(m.isAdmin?"admin":"player");return '<div class="admin-message '+(role==="admin"?"admin-own":"")+'"><small>'+self.Escape(m.name||m.senderName)+' · '+self.Escape(m.time||m.date||"")+'</small><div class="admin-bubble">'+self.Escape(m.text||m.message)+'</div></div>';}).join("")+'</div>'+(owned?'<div class="admin-composer"><div class="admin-quick-wrap"><button type="button" class="admin-quick-toggle" data-admin-quick-toggle>Швидкі відповіді ▾</button>'+quickMenu+'</div><div class="admin-send-row"><textarea class="admin-textarea" data-admin-draft placeholder="Напишіть повідомлення…"></textarea><button type="button" class="admin-send" data-admin-action="send">➤</button></div><div class="admin-actions"><select class="admin-select" data-admin-transfer><option value="">Передати адміну…</option>'+admins.map(function(a){return '<option value="'+self.Escape(a.id)+'">'+self.Escape(a.name)+' ['+self.Escape(a.id)+']</option>';}).join("")+'</select><button type="button" data-admin-action="transfer">Передати</button><button type="button" data-admin-action="release">Звільнити</button></div><button type="button" class="admin-close-ticket" data-admin-action="resolve">Закрити тікет</button></div>':!closed&&r.adminId==null?'<div class="admin-composer"><button type="button" class="admin-primary" data-admin-action="claim">Взяти звернення</button></div>':'<div class="admin-composer admin-muted">Перегляд історії листування</div>');},
>>>>>>> d5fe9faab3c2327fb340c34de655f377d02e845d
    ...
    Click: function(e){
        ...
        if(!b)return;
        if(b.dataset.adminTab){this.tab=b.dataset.adminTab;this.query="";this.chatOpen=false;this.ticketFocus=false;this.Render();return;}
        if(b.dataset.adminFilter){this.filter=b.dataset.adminFilter;this.selectedTicket=null;this.chatOpen=false;this.ticketFocus=false;this.Render();return;}
        if(b.dataset.adminTicket){this.selectedTicket=b.dataset.adminTicket;this.chatOpen=true;this.Render();return;}
        if(b.dataset.adminItems){this.itemCategory=b.dataset.adminItems;this.query="";this.Render();return;}
<<<<<<< HEAD
        if(b.dataset.adminTicketFocus!==undefined){this.ticketFocus=!this.ticketFocus;this.Render();return;}if(b.dataset.adminBack!==undefined){this.chatOpen=false;this.quickOpen=false;this.Render();return;}
        if(b.dataset.adminQuickToggle!==undefined){this.quickOpen=!this.quickOpen;this.Render();return;}
        if(b.dataset.adminQuick!==undefined){var q=this.quickReplies[Number(b.dataset.adminQuick)],d=this.Q("[data-admin-draft]");if(q&&d){d.value=q.text;this.quickOpen=false;d.focus();var menu=this.Q(".admin-quick-menu");if(menu)menu.style.display="none";}return;}
=======
        if(b.dataset.adminQuickToggle!==undefined){this.quickOpen=!this.quickOpen;this.Render();return;}
        if(b.dataset.adminQuick!==undefined){var q=this.quickReplies[Number(b.dataset.adminQuick)],d=this.Q("[data-admin-draft]");if(q&&d){d.value=q.text;this.quickOpen=false;var menu=this.Q(".admin-quick-menu");if(menu)menu.remove();d.focus();}return;}
>>>>>>> d5fe9faab3c2327fb340c34de655f377d02e845d
    ...
    Action: function(action){
        ...
        if(action==="claim"){this.pendingClaimTicket=Number(ticketId);this.Send("admin:ticket:claim",{TicketId:Number(ticketId)});return;}
        if(action==="release"){this.Send("admin:ticket:release",{TicketId:Number(ticketId)});return;}
        if(action==="resolve"){this.Send("admin:ticket:close",{TicketId:Number(ticketId)});return;}
        if(action==="send"){
